...
const CastList = ({ cast }) => {
    ...
    useEffect(() => {
        ...
        if (!scrollContainer || castSearch || isCastExpanded) return; // Don't auto-scroll if searching or expanded
        ...
        const onInteractEnd = () => {
            isInteracting = false;
            // Don't reset scrollPos here, just let it continue from current
            if (scrollContainer) scrollPos = scrollContainer.scrollLeft;
        };

        // Pause on hover/focus for accessibility
        const onPause = () => { isInteracting = true; };
        const onResume = () => { isInteracting = false; if (scrollContainer) scrollPos = scrollContainer.scrollLeft; };
        ...
        // Accessibility: Pause on hover removed per user request (Continuous scroll)

        const scroll = () => {
            if (!isInteracting && scrollContainer) {
                scrollPos += speed;
                // Reset if reached halfway (assuming double content)
        ...
        return () => {
            ...
            if (scrollContainer) {
                ...
                // Removed hover pause listeners per user request
            }
        };
