...
const SharePosterModal = ({ movie, onClose, isDaily }) => {
    ...
    const [orientation, setOrientation] = useState(window.innerWidth >= 768 ? 'landscape' : 'portrait');
    ...
    // Scale factor for DOM Preview (We preview at logical CSS pixels)
    // Portrait: 375x667 (Scale 3) -> 1125x2001
    // Landscape: 640x360? No, let's say logical 960x540 (Scale 2) -> 1920x1080. 
    // Actually simplicity: We just define the logical CSS size for preview.
    const PREVIEW_W = isLandscape ? 800 : 375;
    const PREVIEW_H = isLandscape ? 450 : 667;
    const SCALE = isLandscape ? 1920 / 800 : 3; // 2.4 vs 3
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            const qrImage = qrSrc; // Already loaded object? No, likely recreated state. Use src.
            // Wait, setQrSrc stored the IMAGE OBJECT in state. Correct.
