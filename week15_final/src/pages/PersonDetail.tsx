...
import { getPersonDetails, getImageUrl } from '../services/tmdbClient';
...
const PersonDetail: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchPerson = async () => {
            try {
                if (!id) return;
                const data = await getPersonDetails(parseInt(id, 10));
                if (data) {
                    setPerson(data);
                    try {
    ...
    const [displayedPerson, setDisplayedPerson] = useState<PersonDetails | null>(null);
    ...
    const activePerson = person || displayedPerson;
    ...
    const allCredits = activePerson.movie_credits?.cast || [];
    ...
    return (
        ...
        >
            ...
            <div className="fixed top-6 right-6 md:right-12 z-40 pointer-events-none mix-blend-difference text-xs font-bold text-white hidden md:block">
                PERSONNEL_FILE: {activePerson.id}
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-24">
                    ...
                    <div className="md:col-span-4 lg:col-span-3">
                        <div className="aspect-[2/3] w-full overflow-hidden border border-white/20 bg-white/5 relative">
                            <ImageWithFallback
                                src={getImageUrl(activePerson.profile_path, 'h632')}
                                alt={activePerson.name}
                                className="w-full h-full object-cover"
                            />
                            <div className="absolute bottom-0 left-0 w-full bg-black/80 backdrop-blur-sm p-4 border-t border-white/10">
                                <h1 className="text-xl font-bold uppercase leading-none">{activePerson.name}</h1>
                                <div className="text-[10px] text-white/50 mt-1 uppercase tracking-wider">{activePerson.known_for_department}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-8 lg:col-span-9 space-y-8">
                        ...
                        <div className="space-y-4">
                            ...
                            <p className="text-sm md:text-base text-white/80 leading-relaxed whitespace-pre-line max-w-prose">
                                {activePerson.biography || "NO_BIOGRAPHICAL_DATA_AVAILABLE."}
                            </p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 border-t border-white/10 pt-8">
                            <div>
                                ...
                                <div className="text-sm font-bold">{activePerson.birthday || 'N/A'}</div>
                            </div>
                            <div>
                                <div className="text-[10px] uppercase tracking-widest text-white/40 mb-1">Place of Birth</div>
                                <div className="text-sm font-bold">{activePerson.place_of_birth || 'N/A'}</div>
                            </div>
                            <div>
                                <div className="text-[10px] uppercase tracking-widest text-white/40 mb-1">Known For</div>
                                <div className="text-sm font-bold">{activePerson.known_for_department}</div>
                            </div>
                            <div>
                                <div className="text-[10px] uppercase tracking-widest text-white/40 mb-1">Popularity</div>
                                <div className="text-sm font-bold">{activePerson.popularity.toFixed(0)}</div>
                            </div>
                        </div>
                    </div>
                </div>
